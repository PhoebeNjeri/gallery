...
class PhotographerTestClass(TestCase):
    ...
    # Set up method
    def setUp(self):
        self.Phoebe= Photographer(first_name = 'Phoebe', last_name ='Njeri')
    # Testing  instance
    def test_instance(self):
        self.assertTrue(isinstance(self.Phoebe,Photographer))   
    # Testing Save Method
    def test_save_method(self):
        self.Phoebe.save_photographer()
        photographers = Photographer.objects.all()
        self.assertTrue(len(photographers) > 0) 
    def test_delete_method(self):
        self.Phoebe.save_photographer()
        self.Phoebe.delete_photographer()
        photographers = Photographer.objects.all()
        self.assertTrue(len(photographers) == 0) 
    def test_display_method(self):
        self.Phoebe.save_photographer()
        self.Phoebe.display_photographers()
        photographers = Photographer.objects.all()
        self.assertTrue(len(photographers) > 0) 
